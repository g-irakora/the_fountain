    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1A1A2E 0%, #16213e 60%, #0f3460 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '440px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '32px' }}>
          <h1 style={{ fontFamily: 'Poppins, sans-serif', fontSize: '2rem', color: '#F4A261', letterSpacing: '2px' }}>⛲ FOUNTAIN</h1>
        ...
        <div className="card" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
          ...
          {/* Quick login */}
          <p style={{ fontSize: '0.8rem', color: '#8888aa', marginBottom: '12px', textAlign: 'center' }}>Quick login — try a demo account:</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[
              { label: '🌱 Youth (Amara)', email: 'youth@fountain.com', password: 'youth123', color: '#E8621A' },
              { label: '🎓 Mentor (Dr. Kwame)', email: 'mentor@fountain.com', password: 'mentor123', color: '#2D6A4F' },
              { label: '🏢 Entrepreneur (Zainab)', email: 'entrepreneur@fountain.com', password: 'entre123', color: '#1a73e8' },
            ].map((u) => (
              <button
                key={u.email}
                type="button"
                onClick={() => quickLogin(u.email, u.password)}
                style={{ padding: '8px 12px', border: `1.5px solid ${u.color}`, borderRadius: '8px', background: 'transparent', color: u.color, fontSize: '0.82rem', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s' }}
                onMouseOver={(e) => { e.target.style.background = u.color; e.target.style.color = '#fff'; }}
                onMouseOut={(e) => { e.target.style.background = 'transparent'; e.target.style.color = u.color; }}
              >
                {u.label}
              </button>
            ))}
        ...
        <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '0.78rem', color: 'rgba(255,255,255,0.3)', cursor: 'pointer' }} onClick={() => navigate('/')}>
          ← Back to home
        </p>
      </div>
    </div>
