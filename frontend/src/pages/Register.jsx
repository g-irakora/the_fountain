    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1A1A2E 0%, #16213e 60%, #0f3460 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '500px' }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.9rem', marginTop: '4px' }}>Join thousands of young African creators.</p>
        </div>
        ...
        <div className="card" style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
          ...
          <form onSubmit={submit}>
                ...
                {ROLES.map((r) => (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer', background: form.role === r.value ? '#fff8f3' : '#fff',
                      transition: 'all 0.2s'
                    }}
                  >
                    <div style={{ fontSize: '1.3rem', marginBottom: '4px' }}>{r.label.split(' ')[0]}</div>
                    <div style={{ fontSize: '0.75rem', fontWeight: '700', color: form.role === r.value ? '#E8621A' : '#4a4a6a' }}>
            ...
            <div className="form-group">
              ...
              <input className="form-control" name="skills" value={form.skills} onChange={handle} placeholder="e.g. Marketing, Agriculture, Tech" />
            </div>
            ...
            <button className="btn btn-primary" type="submit" disabled={loading} style={{ width: '100%', padding: '12px', fontSize: '1rem', marginTop: '8px' }}>
              {loading ? 'Creating account…' : 'Join the Fountain →'}
            </button>
          </form>
